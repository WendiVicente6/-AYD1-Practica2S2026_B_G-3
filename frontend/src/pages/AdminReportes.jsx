...
export default function AdminReportes() {
    ...
    useEffect(() => { Promise.all([obtenerSolicitudes(), 
         obtenerHistorial()]).then(([p, h]) => setR({ pendientes: p.length, aprobadas: h.filter(x => x.cod_estado === 2).length,
         rechazadas: h.filter(x => x.cod_estado === 3).length, total: p.length + h.length })).catch(() => setR(obtenerReporteDemo())); }
         , []);
